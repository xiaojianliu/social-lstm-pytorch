...
# Test dataset
parser.add_argument('--visual_dataset', type=int, default=4,
...
save_directory = '/home/hesl/PycharmProjects/social-lstm-pytorch/save/FixedPixel_Normalized_300epoch/'+str(sample_args.visual_dataset)+'/'
...
f = open('/home/hesl/PycharmProjects/social-lstm-pytorch/save/FixedPixel_Normalized_300epoch/'+str(sample_args.visual_dataset)+'/results.pkl', 'rb')
...
for k in range(int((len(data_loader.frameList[0])/skip_frame)/(sample_args.obs_length+sample_args.pred_length))):
    ...
    for j in range(sample_args.obs_length+sample_args.pred_length):
        ...
        cv2.imshow("avatar", avatar)
        imagename='/home/hesl/PycharmProjects/social-lstm-pytorch/plot/FixedPixel_Normalized_300epoch/'+str(sample_args.visual_dataset)+'/visualize-'+ str(int(data_loader.frameList[0][count+j*skip_frame+k*(sample_args.obs_length+sample_args.pred_length)*skip_frame])).zfill(3) + ".jpg"
        #print(imagename)
